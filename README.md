...
デジタル広告市場は7,400億ドルを超える。Google（77%）、Meta（97%）、Amazon（第3の収益柱）——これらの企業の収益構造は「人間がクリックする」という行為に経済的に依存している。

本書の問いは明快だ。**AIエージェントが購買を代行する時、広告モデルはどうなるのか。**

過去の広告モデル移行では、注意の「表面」が変わった——紙からウェブへ、デスクトップからモバイルへ。しかし注意の「主体」は常に人間だった。エージェント商取引では、**注意の主体そのものが人間からAIに移行する**。人間がどこを見るかではなく、人間が見なくなる。

判定ではない。優劣の比較ではない。本書は、**現在進行している構造変化を、観測可能な事実とデータから記述する**ことを目的とするOSS書籍である。
...
| ファイル | 言語 | 内容 |
| --- | --- | --- |
| [agentic-commerce-economy_JP.md](./docs/jp/agentic-commerce-economy_JP.md) | 🇯🇵 日本語 | 本文（日本語版） |
| [agentic-commerce-economy_EN.md](./docs/en/agentic-commerce-economy_EN.md) | 🇺🇸 English | 本文（英語版） |
...
| **The Growth Engine of Anthropic**  | Anthropicの1兆ドル到達の構造解剖                          | [GitHub](https://github.com/Leading-AI-IO/growth-engine-of-anthropic)     |
...
Issues and Pull Requests are welcome. 本書の構造分析に対するフィードバック、Agentic Commerceおよびデジタル広告市場の最新動向の追加情報、誤字脱字の修正など、皆様からのContributeを歓迎します。
